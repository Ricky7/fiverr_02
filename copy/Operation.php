<?php
    ...
    class Operation {
		...
		public function compare_fetch_single($idEje, $idEve)
		{
			...
			// 	"SELECT DATE_FORMAT(S.salesdate,'%m %Y') YearMonth2, COUNT(idStand) SalesReal
			// 	FROM Stand S 
			// 	INNER JOIN Prospect P ON S.idProspect = P.idProspect
			// 	INNER JOIN ProspectExecutiveEvent PEE ON P.idProspect = PEE.idProspect
			// 	WHERE S.status = 2 
			// 	AND PEE.idEvent = 3 
			// 	AND PEE.idEjecutive = 11
			// 	GROUP BY YearMonth2"
			// );
			// $statement2->execute();
			// $result2 = $statement2->fetchAll();
			//$r2 = json_encode($result2);
			// if($statement->rowCount()>0)
   //          {
   //              while($row=$statement->fetch(PDO::FETCH_OBJ))
   //              {
   //              	$output['YearMonth'] = $row->YearMonth;
   //              	$output['goal'] = $row->goal;
   //              	//$return_arr = $output;
   //              	$statement2 = $this->db->prepare(
			// 			"SELECT COUNT(idStand) SalesReal 
			// 			FROM Stand S 
			// 			INNER JOIN Prospect P ON S.idProspect = P.idProspect
			// 			INNER JOIN ProspectExecutiveEvent PEE ON P.idProspect = PEE.idProspect
			// 			WHERE S.status = 2 
			// 			AND PEE.idEvent = 3 
			// 			AND PEE.idEjecutive = 11 
			// 			AND DATE_FORMAT(S.salesdate,'%m %Y') = '{$row->YearMonth}'"
			// 		);
			// 		$statement2->execute();
			// 		//$result2 = $statement2->fetchAll();
   ...
   //          }
            ...
            //echo $merger=json_encode(array_merge_recursive(json_decode($r, true),json_decode($r2, true)));
   //          $r3 = [];
			// foreach(json_decode($r, true) as $key => $array){
			//  $r3[$key] = array_merge_recursive(json_decode($r2, true)[$key],$array);
			// }
			// echo json_encode($r3);
		    //echo json_encode($result);
		    echo json_encode($result);
